...
type NumType = Double
type ParseNum = Parsec String () NumType
...
parseExp :: String -> Either ParseError NumType
...
factor = do
    ...
    x <- try (between (char '(') (char ')') expression) <|> float
...
formatResult :: Either ParseError NumType -> String
